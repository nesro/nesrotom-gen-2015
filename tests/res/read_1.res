TM  simulation (enter h for help)...
Enter command: Tracing now on.
Enter command: Printing instruction count now on.
Enter command:     0:     IN  0,0,0
Enter value for IN instruction:     1:     ST  0,  0(6)
    2:     IN  0,0,0
Enter value for IN instruction:     3:     ST  0,  1(6)
    4:     IN  0,0,0
Enter value for IN instruction:     5:     ST  0,  2(6)
    6:     IN  0,0,0
Enter value for IN instruction:     7:     ST  0,  3(6)
    8:     LD  0,  0(6)
    9:     LD  1,  1(6)
   10:    MUL  0,0,1
   11:     ST  0, 50(6)
   12:     LD  0, 50(6)
   13:     LD  1,  3(6)
   14:    MUL  0,0,1
   15:     LD  1,  2(6)
   16:    MUL  0,0,1
   17:     LD  1, 50(6)
   18:    MUL  0,0,1
   19:     ST  0,  4(6)
   20:     LD  0,  4(6)
   21:    OUT  0,0,0
OUT: 48
   22:   HALT  0,0,0
HALT: 0,0,0
Number of instructions executed = 23
Halted
Enter command: Simulation done.
